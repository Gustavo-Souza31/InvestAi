...
if ($result->num_rows === 0) {
    Logger::log('WARN', 'PASSWORD_RESET_REQUEST', ['motivo' => 'E-mail não encontrado'], 'falha');
...
if ($stmt_update->execute()) {
    Logger::log('WARN', 'PASSWORD_RESET_REQUEST', [], 'sucesso', (int) $usuario['id'], $email);
    echo json_encode(['status' => 'success', 'message' => 'Senha alterada com sucesso, você já pode fazer login.']);
} else {
    Logger::log('ERROR', 'PASSWORD_RESET_REQUEST', ['motivo' => 'Erro ao atualizar no banco'], 'falha', (int) $usuario['id'], $email);
